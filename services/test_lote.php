...
foreach($files as $name) {
    ...
    $csv_file = "tmp/csv/".$ano.$mes.$dia.$hora.$min.'.csv';

        http://bodysystems.net/_ferramentas/dashboard-iso/services/lote_xls.php?f=201404281600.jsn
    ...
    $query = "SELECT des1.nsa, bol.transacaoid, bol.cnab AS cnab, bol.idcliente, bol.userid, bol.nome_evo, bol.user_email, bol.endereco_remessa, bol.valor_frete, des1.dta_liberacao, des1.eventoid,'BB' as origem
                    ...
                    INNER JOIN despacho_controle AS des2 ON des2.eventoid = SUBSTR(PagS.ProdID,2,5) WHERE PagS.StatusTransacao IN ('Aprovado','Completo')  AND PagS.TransacaoID IN (SELECT despacho.transacaoid FROM despacho WHERE lote LIKE '$lote')
    ...
    $sHead  = "EVO_ID;SITE_ID;CLIENTE;ENDERECO;FRETE"."\n";
    foreach($result as $obj) {
        $line .= $obj->idcliente.";".$obj->userid.";".$obj->nome_evo.";".utf8_decode($obj->endereco_remessa).";".$obj->valor_frete."\n" ;
    }

    $ret = file_put_contents ($csv_file,$sHead.$line) ;
